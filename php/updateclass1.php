       ...
       $db=mysql_select_db("cms",$con);	
        if(!$db)				
         { 
          ...
          $str="DELETE  FROM allocate WHERE class_id='$cid'";  
          $query=mysql_query($str); 
           if($query)
           {
            $str1="UPDATE `class` SET `status`=0 WHERE class_id='$cid'";  
            $query1=mysql_query($str1); 
           if($query1)
           {
           echo " ***SUCCESSFULLY DELETED***" . $cid;
           
           }
           else{
            echo "Cannot Delete from class";
           }
          
           }
           else{
            echo "cannot delete from allocate";
           }
          }
